...
class AIModelNode(Node):
    ...
    def extract_features(self, image):
        """ResNet'i kullanarak görüntüden öznitelik çıkarır (2048 boyutlu)"""
        ...
        with torch.no_grad():
            ...
            features = torch.flatten(features, 1)  # Özellikleri düzleştir (1, 2048)

        return features.cpu().numpy().squeeze()

    def publish_image(self, cv_image):
        """İşlenmiş görüntüyü ROS topic'ine yayınla"""
        try:
            img_msg = self.bridge.cv2_to_imgmsg(cv_image, "bgr8")
            self.image_publisher.publish(img_msg)
            self.get_logger().info("İşlenmiş görüntü yayınlandı.")
        except Exception as e:
